...
| V0.1.5     | 增加依赖            | 徐昀       | 2026-08-18   | 增加「千里绘江山」可选依赖，支持自动生成配色方案             |
...
- **千里绘江山** — 若已加载，在生成文档时可自动调用其配色生成能力，为文档提供与风格匹配的定制化配色方案，替代内置固定样式色值，提升视觉统一性。

> 💡 建议加载“千里绘江山”以获得自动配色能力。若未加载，文档生成核心功能仍可正常运行，使用插件内置的固定样式。*
...
- **如果已加载** → 输出提示：“✅ 可选依赖已加载：千里绘江山 → 文档生成时可自动调用配色生成能力，为您提供定制化配色方案。是否启用配色生成？（输入‘是’启用，或‘跳过’使用内置样式）”
  - 用户选择“是” → 生成文档时调用千里绘江山生成配色
  - 用户选择“跳过” → 使用插件内置固定样式

- **如果未加载** → 输出提示：“💡 可选依赖未加载：千里绘江山。若需要自动生成与文档风格匹配的定制化配色，建议加载。如忽略，将使用插件内置样式。”
