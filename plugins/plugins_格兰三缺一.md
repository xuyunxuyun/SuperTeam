...
| V0.1.2     | 内容修订         | 盘古开插件 | 2026-06-01   | 增加完整工作流程（含Mermaid图）；优化JSON质量评估;自适应跳数扩展；全员反馈优化 |
| V0.1.3     | 内容修订         | 徐昀       | 2026-08-14   | 优化检索规则                                                 |
...
| 阶段      | 触发条件          | 跳数上限变化 | 是否告知用户         | 告知内容                       |
| :-------- | :---------------- | :----------- | :------------------- | ------------------------------ |
| 第1轮     | 默认              | 3跳          | 不告知               | —                              |
| 第2轮     | 3跳无结果         | →5跳         | ❌ 不告知（静默升级） | —                              |
| 第3轮     | 5跳无结果         | →7跳         | ✅ 告知               | “范围已扩大”                   |
| 第4轮     | 7跳无结果         | →10跳        | ✅ 告知               | “继续扩大范围”                 |
| 第5轮     | 10跳无结果        | —            | ✅ 告知               | “未找到，已尝试3/5/7/10跳”     |
| **补充1** | 3跳有结果但不完整 | →7跳         | ✅ 告知               | “信息可能不完整，扩大范围补充” |
| **补充2** | 5跳有结果但不完整 | →7跳         | ✅ 告知               | “信息可能不完整，扩大范围补充” |
| **补充3** | 7跳有结果但不完整 | →10跳        | ✅ 告知               | “继续扩大范围补充”             |
| **补充4** | 10跳仍不完整      | —            | ✅ 告知               | “已检索到10跳，以下为部分信息” |
...
flowchart TB
    ...
    subgraph Phase1[❓ 阶段1：提问与转化]
        ...
        B1 --> B2[哈利：按优先级匹配节点<br/>name→label→props→关系]
        B2 --> B3{匹配结果}
    ...
    subgraph Phase2[🔍 阶段2：检索执行（自适应跳数）]
        ...
        C2 -- 否 --> C3{当前跳数上限 N}
        C3 -- N ≤ 5 --> C4[静默升级<br/>N = N+2与10较小值]
        C4 -.-> C0
        C3 -- N = 7 --> C5[告知用户<br/>范围已扩大]
        C5 --> C4
        C3 -- N = 9 --> C6[继续升级<br/>N = 10]
        C6 -.-> C0
        C3 -- N = 10 --> C7[罗恩：告知未找到<br/>附已尝试跳数范围]
        C7 --> C8[流程结束]
        
        C2 -- 是 --> C9{答案是否完整？}
        C9 -- 完整 --> C10[进入审查阶段]
        C9 -- 不完整且N<10 --> C11[告知用户 扩大范围补充<br/>N = N+2与10较小值]
        C11 -.-> C0
        C9 -- 不完整且N=10 --> C12[输出已有结果<br/>标注 部分信息]
        C12 --> C10
    end
...
| 1.3  | 哈利   | 按优先级匹配节点：`name → label → props → 关系`  | 起始节点候选 |
...
**注**：升级公式为 `N = min(N+2, 10)`，因此从7跳无结果会直接跳到10跳，不会出现N=9的死分支。

**答案完整性判断：**

| 当前跳数 N | 信息完整度 | 处理方式                         | 是否告知用户                       |
| :--------- | :--------- | :------------------------------- | :--------------------------------- |
| N=3        | 不完整     | 自动扩展到7跳，重新检索          | 告知“信息可能不完整，扩大范围补充” |
| N=5        | 不完整     | 自动扩展到7跳，重新检索          | 告知“信息可能不完整，扩大范围补充” |
| N=7        | 不完整     | 自动扩展到10跳，重新检索         | 告知“继续扩大范围补充”             |
| N=10       | 仍不完整   | 输出已有结果，标注“**部分信息**” | 告知“已检索到10跳，以下为部分信息” |

**注**：如果检索无结果，走“无结果”分支（3→5→7→10），不进入“答案不完整”分支。两种分支的跳数扩展逻辑一致，均使用公式 `N = min(N+2, 10)`。
